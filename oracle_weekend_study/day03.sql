...
SELECT * FROM PLAYER
WHERE TEAM_ID != 'K01';
/*PLYAER 테이블에서 WEIGHT가 70이상이고 80이하인 선수*/
SELECT * FROM PLAYER
...
/*PLYAER 테이블에서 TEAM_ID가 'K06'이고 NICKNAME이 '제리'인 선수*/
SELECT * FROM PLAYER
WHERE TEAM_ID = 'K06' AND NICKNAME = '제리';
/*PLYAER 테이블에서 HEIGHT가 170이상이고 WEIGHT가 80이상인 선수 이름*/
SELECT * FROM PLAYER
...
/*PLYAER 테이블에서 TEAM_ID가 'K02'이거나 'K07'이고 포지션은 'MF'인 선수*/
SELECT * FROM PLAYER
...
SELECT PLAYER_NAME, NVL("POSITION", '미정') "POSITION"  FROM PLAYER p 
WHERE "POSITION" IS NULL;
/*PLAYER 테이블에서 NATION이 등록되어 있으면 '등록', 아니면 '미등록'으로 검색*/
SELECT PLAYER_NAME, NVL2(NATION, '등록', '미등록') NATION  FROM PLAYER p;
SELECT PLAYER_NAME, NVL2(NATION, NATION , '미등록') 국가 FROM PLAYER p;

/*AS(ALIAS) : 별칭*/
/*SELECT 절에서는 컬럼명 뒤에 띄어쓰고 작성하거나, AS 뒤에 작성한다.*/
...
/*선수 이름과 생일 조회*/
SELECT PLAYER_NAME "선수 이름", BIRTH_DATE 생일 FROM PLAYER p;

/*PLAYER 테이블에서 BACK_NO를 "등 번호"로, NICKNAME을 "선수 별명"으로 변경하여 검색*/
SELECT PLAYER_NAME, BACK_NO "등 번호", NICKNAME "선수 별명" FROM PLAYER p;

/*위 결과에서 NICKNAME이 NULL일 경우 없음으로 변경*/
SELECT PLAYER_NAME, BACK_NO "등 번호", NVL(NICKNAME, '없음') "선수 별명" FROM PLAYER p;
...
/*PLAYER 테이블에 이씨가 아닌 사람 찾기(NOT) WHERE 뒤에나 LIKE 앞에 NOT*/
SELECT * FROM PLAYER p 
...
/*
 * GROUP BY: ~별 (예: 포지션 별 평균 키) -> 묶어야 할 때 사용
 * 
...
SELECT "POSITION", AVG(HEIGHT), AVG(WEIGHT), MIN(WEIGHT) FROM PLAYER 
WHERE WEIGHT >= 80
GROUP BY "POSITION"
HAVING AVG(HEIGHT) >= 180; 
...
/*경기장 중 경기 일정이 20120501~20120505 사이에 있는 경기장 전체 정보 조회*/
SELECT * FROM SCHEDULE;

SELECT * FROM STADIUM
WHERE STADIUM_ID IN (
	SELECT DISTINCT STADIUM_ID FROM SCHEDULE
